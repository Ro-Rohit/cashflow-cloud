...
const requiredOption = ['amount', 'Duedate', 'name'];
...
const ImportCard: NextPage<Props> = ({ onCancel, data, onSubmit }) => {
  ...
  const handleContinue = () => {
    ...
    const formattedData = arrayOfData
      .map((row: any) => {
        try {
          const parsedDate = parseDateWithFormats(row.Duedate, possibleDateFormats);
          if (!parsedDate) throw new Error(`Invalid date format: ${row.Duedate}`);

          return {
            name: row.name,
            amount: convertAmountToMiliUnit(row.amount),
            dueDate: format(parsedDate, outputFormat),
          };
        } catch (err) {
          toast.error(`Invalid date format for date: ${row.Duedate}`);
    ...
    <Card className="border border-primary max-w-screen-xl pb-10 mx-5 lg:mx-auto shadow-md p-4 mb-10 mt-[120px]  sm:mt-[-140px]">
      <CardHeader className="flex flex-col sm:flex-row gap-y-1 items-center justify-between">
        ...
        <div className="flex flex-col sm:flex-row gap-1  w-full sm:w-auto">
          ...
          <Button disabled={progress < 3} onClick={handleContinue} className="w-full sm:w-auto">
